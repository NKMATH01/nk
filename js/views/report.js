...
async function renderReport(c){
  ...
  const readinessCell=readiness!=null
    ...
    :(useSnap?'<span class="val-wait">산출 대기</span>':'-');
  const readinessNote=useSnap
    ?(snap&&snap.snap_date
      ?`<div class="card-cap tight">${esc(fmtDate(snap.snap_date))} 산출 기준</div>`
      :'<div class="card-cap tight">아직 산출되지 않았습니다. 강사가 확인 후 반영됩니다.</div>')
    :'';
  // 최근 8주 처방 성과 요약
  ...
  const nextSteps=`<div>${weak
    ...
    <div class="step-grp"><b>진행 중 보완 과제</b>${rxActive.length?` ${rxActive.length}건`:''}</div>
    ${rxActive.length?rxActive.map(p=>`<div class="step-item">· ${esc(p.unit)} · ${esc(p.cognition)}${p.note?' — '+esc(p.note):''}${p.due_date?` <span class="muted">기한 ${esc(fmtDate(p.due_date))}</span>`:''}</div>`).join('')
      :'<div class="step-item">· 현재 배정된 보완 과제가 없습니다.</div>'}
    <div class="step-grp">${nextSess
  ...
  const pastList=comments.length?comments.map(t=>`<div class="panel"><div class="th"><div><span class="chip ${t.week_no===0?'purple':'gray'}">${esc(wkLabel(t.week_no))}</span></div></div>
      <div class="prose">${esc(t.comment)}</div></div>`).join(''):'<p class="muted">등록된 코멘트가 없습니다.</p>';

  c.innerHTML=(readonly?'':await studentSelector())+printBar+`
    <div class="card" id="reportSheet">
      <div class="sheet-head">
        <div><div class="sheet-org">NK 약술논술 AI 케어 · 진행 현황 리포트</div>
          <h2>${esc(st?.name||'')} <span class="muted" style="font-size:var(--fs-lg)">${esc(st?.grade_type||'')}</span></h2></div>
      ...
      <div class="kpi-grid" style="margin-bottom:var(--sp-4)">
        ...
        <div class="kpi"><div class="lbl">최근 회차 점수</div><div class="val">${curPct==null?'-':r1(curPct)+'<small>%</small>'} ${delta==null?'':(delta>=0?'<span class="delta up val-delta">'+svg('arrowUp','xs')+r1(Math.abs(delta))+'%p</span>':'<span class="delta down val-delta">'+svg('arrowDown','xs')+r1(Math.abs(delta))+'%p</span>')}</div></div>
      ...
      <div class="card-cap lead">과제 정답률(성실성): <b>${rd.diligence==null?'-':r1(rd.diligence)+'%'}</b> · 과제 평균 풀이 시간: <b>${hwTime==null?'-':r1(hwTime)+'분/회'}</b> · 진도 커버리지: <b>${coverage==null?'-':Math.round(coverage)+'%'}</b></div>
      ...
      ${readonly?'':`<div class="card-cap lead">회차 난이도 대비(참고): <b>${rd.weeklyScaledEwma==null?'비교군 부족':r1(rd.weeklyScaledEwma)}</b> — 같은 회차를 본 학생들 평균이 50, 10 차이가 표준편차 1개입니다. 준비도 점수에는 넣지 않습니다.</div>`}
      <div class="card-cap lead">처방 성과: ${rxLine}</div>
      <div class="grid2">
        <div><h3 class="sec-ttl">주간 점수 추이</h3><canvas id="repLine" height="150"></canvas></div>
        <div><h3 class="sec-ttl">단원별 정답률</h3>${UNITS.map(u=>{const a=unitAgg[u];const p=a.p>0?Math.round(a.e/a.p*100):null;
      ...
      <div class="sheet-sec"><h3 class="sec-ttl">강사 코멘트</h3>
        ${readonly?'':`<textarea id="rep_comment" class="ta-block" rows="3" placeholder="진행 현황 종합 코멘트">${curComment?esc(curComment.comment):''}</textarea>
            <div class="act-row no-print"><button class="btn sm" id="rep_save">${svg('check','xs')}코멘트 저장</button>
            <button class="btn line sm" id="rep_ai">${svg('spark','xs')}AI 초안</button><span id="rep_msg" class="msg" style="margin-top:0"></span></div>
            <div id="rep_draft" class="no-print"></div>`}
        <div class="cmt-list">${pastList}</div>
      </div>
      <div class="sheet-sec"><h3 class="sec-ttl">다음 단계 안내</h3>
        <div class="note-blue">${nextSteps}</div></div>
      <div class="divider"></div>
      <p class="card-cap">본 리포트는 학습 관리용 참고 자료이며 합격을 보장하지 않습니다. · 생성일 ${todayStr()}</p>
    </div>`;
...
async function bindAiDraft(sid,curComment){
  ...
  if(curComment&&curComment.ai_draft){
    show(`<div class="draft-box"><div class="draft-ttl">이전 AI 초안 (미확정)</div>
      <div class="ai_draft_text">${esc(curComment.ai_draft)}</div>
      <div class="act-row"><button type="button" class="btn sm ai_take">입력란으로 가져오기</button></div></div>`);
    bindTake();
  }
  ...
  btn.addEventListener('click',async()=>{
    ...
    if(app.DEMO){show('<div class="draft-box"><span class="chip gray">데모 모드 — 초안을 생성하지 않습니다</span></div>');return;}
    const status=await db.getAiStatus();
    if(!status||!status.configured){
      show('<div class="draft-box"><span class="chip amber">AI 미설정</span>'
        +'<div class="card-cap tight">GEMINI_API_KEY 가 등록되지 않았습니다. [설정] 화면을 참고하세요. 코멘트는 직접 작성하시면 됩니다.</div></div>');
      return;
    }
    btn.disabled=true;
    show('<div class="draft-box"><span class="card-cap tight">초안 생성 중...</span></div>');
    try{
      const r=await db.generateReportDraft(sid,0);
      show(`<div class="draft-box"><div class="draft-ttl">AI 초안 (미확정 · 강사 확인 필요)</div>
        <div class="ai_draft_text">${esc(r.draft||'')}</div>
        <div class="act-row"><button type="button" class="btn sm ai_take">입력란으로 가져오기</button>
        <span class="card-cap tight">가져온 뒤 수정하고 [코멘트 저장]을 눌러야 리포트에 실립니다.</span></div></div>`);
      bindTake();
    }catch(e){
      show('<div class="draft-box"><div class="msg err">'+esc(e?.message||'오류')+'</div>'
        +'<div class="card-cap tight">검수 기준(확률·합격 단정 표현, 근거 없는 숫자)에 걸리면 초안을 내보내지 않습니다.</div></div>');
