...
/* counseling-audio 버킷에서 객체 하나를 지운다.
   0016(delete)·0022(select) 정책이 **둘 다** 적용돼 있어야 한다 — PostgreSQL 은
   WHERE·RETURNING 이 붙은 DELETE 에 대해 대상 행이 select 정책도 통과해야 지우게
   하는데, Storage 의 remove() 가 바로 그런 DELETE 를 사용자 권한으로 실행한다.
   0016 만 있던 동안 [삭제]·[음성 삭제] 전부가 이 요건에 걸려 실패했다(0022 로 수복).
   실패하면 던진다 — 부르는 쪽은 이 결과를 확인하기 전에 audio_path 를 지우면 안 된다.

   ★ error 만 보면 안 된다. remove() 는 **RLS 가 허용한 것만** 지우고 지워진 목록을
     돌려준다. 정책이 모자라면 아무것도 지우지 않은 채 error 없이 빈 배열이 온다 —
     그대로 통과시키면 "지웠다고 믿는데 버킷에는 남아 있는" 바로 그 상태가 된다.
     그래서 빈 결과도 실패로 본다.
   그 대가로, 객체가 이미 없는 경로(부분 실패 후 재시도 등)도 실패로 잡힌다.
   그 상태가 되면 [삭제]·[음성 삭제] 가 둘 다 막히므로(둘 다 이 함수를 거친다)
   화면에서 풀 방법이 없다 — 0016·0022 적용 여부를 먼저 확인하고, 정말 객체만
...
async function removeCounselAudio(path){
  ...
  if(!data||!data.length)throw new Error('음성 파일을 지우지 못했습니다(대상 없음 또는 권한 없음). 마이그레이션 0016·0022 가 적용되었는지 [설정] 화면의 DB 마이그레이션 상태에서 확인하세요.');
}
